...
export const detectApiVersion = async () => {
  ...
  if (__DEV__) {
    resolvedBaseUrl = Platform.OS === 'web' ? 'http://localhost:5001' : 'http://192.168.0.198:5001';
  }
};
...
export const apiFetch = async (path: string, options: RequestInit = {}) => {
  ...
  // Build candidate URLs (primary -> live double-api fix -> local fallback)
  ...
  if (!primaryUrl.includes('192.168.0.198') && !primaryUrl.includes('localhost')) {
    const localHost = Platform.OS === 'web' ? 'http://localhost:5001' : 'http://192.168.0.198:5001';
    const localPath = path.startsWith('/api/') ? path : `/api${path.startsWith('/') ? path : `/${path}`}`;
    candidateUrls.push(`${localHost}${localPath}`);
  }
